...
from src.jonaspage import JonasPage
...
def scrape_manuscripts(
    infile: str, column_name: str, database: str, restart: bool, redo: bool
):
    console = Console()
    console.clear()
    task = Panel(
        Text(f"URL dataset: '{infile}'", justify="center", style="italic"),
        title="Scraping manuscript pages",
        padding=1,
    )
    console.print(task)
    ...
    jonas_urls = [
        t[0]
        for t in duckdb.read_csv(infile).aggregate(column_name, column_name).fetchall()
        if t[0] is not None
    ]
    expected_length = len(jonas_urls)

    db = Database(persistent_file=database, restart=restart)
    ...
    table1.add_column(
        f"Unique values in '{column_name}'", justify="right", style="cyan", no_wrap=True
    )
    table1.add_column(str(len(jonas_urls)), justify="right", style="red")
    ...
    console.print(f"DuckDB database file: {database}\n")
